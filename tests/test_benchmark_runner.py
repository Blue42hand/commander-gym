...
class BenchmarkRunnerTests(unittest.TestCase):
    def test_reports_scores_categories_usage_and_pilot_provenance(self):
        ...
        def choose(case: BenchmarkCase) -> PilotDecision:
            action_id = "cast" if case.case_id == "case-a" else "pass"
    ...
    def test_baseline_helpers_stay_within_fixture_legal_actions(self):
        case = synthetic_case("case-helper", "baseline")

        first = run_benchmark([case], first_legal_pilot())
        oracle = run_benchmark([case], preferred_or_first_pilot())

        self.assertEqual(first["cases"][0]["selected_action_id"], "pass")
        self.assertTrue(first["cases"][0]["legal"])
        self.assertEqual(oracle["cases"][0]["selected_action_id"], "cast")
        self.assertTrue(oracle["cases"][0]["preferred"])
